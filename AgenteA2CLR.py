...
def AgenteA2CLRTrain(model_name, train_data, l_r=0.0007, total_timesteps=1000000, inversion = 100000, cv_cost = 0.0025, custody_cost = 0.0015, verbose = 0):
    # Creo el entorno
    ...
    model = A2C(MultiInputPolicy, env, learning_rate=l_r, verbose=0, tensorboard_log=log_dir)

    if verbose > 0:
        print("Empezando a entrenar modelo A2C_{}".format(model_name))
    
    model.learn(total_timesteps,tb_log_name="A2C_lr_" + "{:.4f}".format(l_r), progress_bar=True)
