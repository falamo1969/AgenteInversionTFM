...
def AgentePPOTrain(model_name, train_data, total_timesteps=1000000, l_r = 0.0003, inversion = 100000, cv_cost = 0.0025, custody_cost = 0.0015, verbose = 0):
    # Creo el entorno
    ...
    model = PPO(MultiInputPolicy, env, learning_rate=l_r, verbose=0, tensorboard_log=log_dir)
    ...
    model.save("Models/PPO" + model_name +"_LR_{:.4f}".format(l_r)+"_data.model")
    return model
...
def AgentePPOTest(model, test_data, results_file, n_episodes=10, inversion = 100000, cv_cost = 0.0025, custody_cost = 0.0015, verbose = 0):
    ...
    # Salvo resultados
    pickle.dump(agente_PPO_pickle,open('./results/PPO/PPO_'+ results_file +'.pickle', 'wb'))    
